...
class VGG_SNN_STDB(nn.Module):
	...
	def insert_faults(self, x):
		...
		for l in range(len(self.features)):
			if isinstance(self.features[l], nn.Conv2d):
				# self.fault[l] 		= torch.ones(self.batch_size, self.features[l].out_channels, self.width, self.height)
				self.f_threshold[l] 		= self.threshold[l]*torch.ones(self.batch_size, self.features[l].out_channels, self.width, self.height)
				l_idx = l + 0.0
				print(self.threshold[l])
				mem_size = list(self.f_threshold[l_idx].size())
				mem_n_idx = len(mem_size)
				mem_n_elem = torch.numel(self.f_threshold[l_idx])
				mem_n_faults = int(mem_n_elem*self.frate)
				mem_i = 0
				while (mem_i < mem_n_faults):
					mem_rand_idx = np.random.randint(0, high=mem_n_elem, size = mem_n_idx)
					mem_fidx = tuple(np.remainder(mem_rand_idx, mem_size))
					mem_i += 1
					try:
						if self.ftype == 'stuck-at-zero':
							# self.fault[l_idx].data[mem_fidx] = 0.0
							self.f_threshold[l_idx].data[mem_fidx] = 0.0000000
						elif self.ftype == 'stuck-at-one':
							 # self.fault[l_idx].data[mem_fidx] = 2.0
							self.f_threshold[l_idx].data[mem_fidx] = 4.0+self.threshold[l_idx]
					except:
						print(self.f_threshold[l_idx].size())
						print("Fail to insert faults")
						print(l_idx)
						print(mem_fidx)
			elif isinstance(self.features[l], nn.AvgPool2d):
				self.width = self.width//self.features[l].kernel_size
		...
		for l in range(len(self.classifier)-1):
			if isinstance(self.classifier[l], nn.Linear):
				# self.fault[prev+l] 		= torch.ones(self.batch_size, self.classifier[l].out_features)
				self.f_threshold[prev+l] 		= self.threshold[prev+l]*torch.ones(self.batch_size, self.classifier[l].out_features)
				l_idx = prev+l
				mem_size = list(self.f_threshold[l_idx].size())
				# mem_size = list([self.batch_size, self.classifier[l].out_features])
				mem_n_idx = len(mem_size)
				# mem_n_idx = 2
				mem_n_elem = torch.numel(self.f_threshold[l_idx])
				mem_n_faults = int(mem_n_elem*self.frate)
				...
				while (mem_i < mem_n_faults):
					...
					try:
						if self.ftype == 'stuck-at-zero':
							# self.fault[l_idx].data[mem_fidx] = 0.0
							self.f_threshold[l_idx].data[mem_fidx] = 0.0000001
						elif self.ftype == 'stuck-at-one':
							# self.fault[l_idx].data[mem_fidx] = 2.0
							self.f_threshold[l_idx].data[mem_fidx] = 4.0+self.threshold[l_idx]
					except:
		...
		for t in range(self.timesteps):
			...
			for l in range(len(self.features)):
				...
				if isinstance(self.features[l], (nn.Conv2d)):
					...
					# self.insert_faults(l)
					try:
						mem_thr 		= (torch.div(self.mem[l],self.f_threshold[l])) - 1.0# 11
						# print("----------> nothing here")
						# print(l)
						# print(self.mem[l].size())
						# print(self.f_threshold[l].size())
						# print(self.threshold[l])
					except:
						print("----------> error here")
						print(l)
					...
					rst 			= self.f_threshold[l]* (mem_thr>0).float()
			...
			for l in range(len(self.classifier)-1):
				...
				if isinstance(self.classifier[l], (nn.Linear)):
					...
					# self.insert_faults(prev+l)
					# mem_thr 			= ((self.mem[prev+l]*self.fault[prev+l])/self.threshold[prev+l]) - 1.0
					mem_thr 		= (torch.div(self.mem[prev+l],self.f_threshold[prev+l])) - 1.0# 11
					out 				= self.act_func(mem_thr, (t-1-self.spike[prev+l]))
					rst 				= self.f_threshold[prev+l] * (mem_thr>0).float()
